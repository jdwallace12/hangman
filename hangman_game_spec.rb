...
describe Game do   
  let!(:game) { Game.new(['John','jason'], "trout")}
  ...
  describe "game initialization" do
    ...
    it "grabs a word" do
      expect(game.word).to eql("trout")
    end
  end

  describe "create blacked out word to hide letter" do
    it "returns a fully blankeded out word" do
      expect(game.blanked).to eql("_____")
    end
  end
  ...
  describe "guessed letters" do
    ...
    it "says that the guessed letter is in the word" do
      expect(game.check_word_for("t")).to be_true
    end

    it "fills in the guessed letter everytime it occurs in the word" do
      game.guess_letter("t")
      expect(game.blanked).to include("t___t")
    end
  end
  ...
  describe "guessing the word" do
    ...
    it "allows correct words to be guessed" do
      incorrect_word = 'river'
      expect(game.correct_word?(correct_word)).to be_false
    end
  end

  describe "player turn" do
    it "returns the first player and cycles through" do
      expect(game.select_player).to eql("John")
